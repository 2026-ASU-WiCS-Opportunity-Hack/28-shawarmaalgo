...
const initialFormState: CreateChapterFormState = {
  name: 'WIAL Canada',
  slug: 'canada',
  country: 'Canada',
  region: 'North America',
  description:
    'WIAL Canada extends the global WIAL mission locally through Action Learning programming, coach development, and chapter-based engagement.',
  description_local: '',
  primary_language: 'English',
  supported_languages: 'English',
  timezone: 'America/Toronto',
  currency: 'CAD',
  contact_email: 'canada@wial.org',
  contact_phone: '',
  contact_city: '',
  website_url: '',
  logo_url: '',
...
export default function NewChapterPage() {
  ...
  const [form, setForm] = useState<CreateChapterFormState>(initialFormState);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  function updateField<K extends keyof CreateChapterFormState>(key: K, value: CreateChapterFormState[K]) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!token) {
      setError('You must be signed in as a super admin to create a chapter.');
      return;
    }
    ...
    const payload: ChapterCreatePayload = {
      name: form.name.trim(),
      slug: form.slug.trim(),
    ...
    try {
      const chapter = await api.createChapter(payload, token);
      router.push(`/portal/admin/chapters/${chapter.slug}`);
      router.refresh();
    } catch (submitError) {
      setError(submitError instanceof Error ? submitError.message : 'Unable to create chapter');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      <Panel title="New chapter details" description="All required backend chapter fields are collected here. Chapter-leader assignment remains a separate step.">
        <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          <label className="block">
            <span className={labelClassName}>Contact city</span>
            <input
              className={inputClassName}
              value={form.contact_city}
              onChange={(event) => updateField('contact_city', event.target.value)}
            />
          </label>
          <label className="block">
          ...
          <label className="block">
            ...
            <input
              ...
              onChange={(event) => updateField('member_count', event.target.value)}
            />
          </label>
