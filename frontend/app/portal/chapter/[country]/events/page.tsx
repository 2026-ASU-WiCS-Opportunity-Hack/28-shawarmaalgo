...
export default async function ChapterEventsPage({ params }: { params: { country: string } }) {
  ...
  if (!chapter) notFound();
  return (
    <PortalShell roleScope="chapter" eyebrow="Chapter leader console" title={`${chapter.name} events`} description="Create new chapter events, revise schedules, and publish local chapter sessions to the public site and the wider WIAL event experience." chapterSlug={chapter.slug}>
