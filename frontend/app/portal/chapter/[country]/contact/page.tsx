...
export default async function ChapterContactPage({ params }: { params: { country: string } }) {
  ...
  if (!chapter) notFound();
  return (
    <PortalShell roleScope="chapter" eyebrow="Chapter leader console" title={`${chapter.name} contact details`} description="Update the local email, phone number, city, and any additional chapter-specific contact messaging shown to visitors." chapterSlug={chapter.slug}>
